...
class Bird:
    ...
    def update(self, scr, txt=None):
        ...
        if scr.maze_map[self.xy[1]][self.xy[0]] != 1:
            self.rct.center = (self.xy[0]*self.tile_size+(self.tile_size//2),self.xy[1]*self.tile_size+(self.tile_size//2))
        else:
            self.xy = copy.deepcopy(tmp)
...
class Bear(Bird):
    ...
    def update(self, scr:Screen):
        ...
        if self.count > self.interval: #既定の間隔で
            self.now_dir, x, y = self.search_left(scr.maze_map) 
            self.xy = [x,y] #くまを動かす
            self.count = 0
            # 以下は同様
...
class main():
    def __init__(self) -> None:
        ...
        while running:
            # クマの増殖判定
            if counter > interval//stage_count:
            ...
            if flg == True: 
                ...
                return
            #ゴール判定
            if kkt.xy == [self.scr.goal_pos[0],self.scr.goal_pos[1]]:
                self.scr.reset_maze()
                counter = 0
                stage_count+=1
                ...
                for i in range(len(brs)-1,-1,-1):
                    # print(i)
                    if i == 0:
                        brs[i].xy = [self.scr.goal_pos[0],self.scr.goal_pos[1]]
                        brs[i].rct.center = (int(brs[i].xy[0])*brs[i].tile_size+(brs[i].tile_size//2),int(brs[i].xy[1])*brs[i].tile_size+(brs[i].tile_size//2))
                    else:
                        brs.pop(-1)
                        i -= 1
                kkt.xy = [1, 1]
                kkt.rct.center = kkt.xy[0]*kkt.tile_size+(kkt.tile_size//2),kkt.xy[1]*kkt.tile_size+(kkt.tile_size//2)
